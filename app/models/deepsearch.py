...
class DeepSearchEvent(BaseModel):
    """DeepSearch 流式事件统一结构。"""
    eventType: DeepSearchEventType = Field(..., description="事件类型")
    timestamp: str = Field(..., description="时间戳（ISO 8601格式）")
    sequenceNumber: int = Field(..., description="事件序号")
...
class ProgressEvent(BaseModel):
    """进度事件数据。"""
    currentStep: str = Field(..., description="当前步骤描述")
    totalSteps: int = Field(..., description="总步骤数")
    completedSteps: int = Field(..., description="已完成步骤数")
    percentage: float = Field(..., description="完成百分比", ge=0.0, le=100.0)


class ResearchPlanEventData(BaseModel):
    """研究计划事件数据。"""
    researchTopic: str = Field(..., description="研究主题")
    subTopics: List[str] = Field(default_factory=list, description="子主题列表")
    researchQuestions: List[str] = Field(default_factory=list, description="研究问题列表")
...
class ReflectionEventData(BaseModel):
    """反思事件数据。"""
    loopCount: int = Field(..., description="当前循环次数")
    is_sufficient: bool = Field(..., description="信息是否充足")
    knowledgeGap: Optional[str] = Field(default=None, description="知识差距描述")
    followUpQueries: List[str] = Field(default_factory=list, description="后续查询列表")


class DeepSearchRequest(BaseModel):
    """DeepSearch 请求模型。"""
    query: str = Field(..., description="研究主题/问题", min_length=1, max_length=8000)
    initialSearchQueryCount: Optional[int] = Field(
        default=None, description="初始搜索 Query 数量（不传则用默认配置）", ge=1, le=10
    )
    maxResearchLoops: Optional[int] = Field(
        default=None, description="最大研究循环次数（不传则用默认配置）", ge=1, le=5
    )
    reasoningModel: Optional[str] = Field(
        default=None, description="用于反思/总结的模型覆盖（不传则用默认配置）"
    )
    reportFormat: ReportFormat = Field(
...
class DeepSource(BaseModel):
    ...
    shortUrl: Optional[str] = Field(default=None, description="短链（正文中用于引用）")
...
class DeepSearchResponse(BaseModel):
    ...
    markdownReport: str = Field(..., description="Markdown格式的完整报告")
    sources: List[DeepSource] = Field(default_factory=list, description="被使用的数据源列表（被引用的）")
    allSources: List[DeepSource] = Field(default_factory=list, description="所有搜索到的网络资源列表")
